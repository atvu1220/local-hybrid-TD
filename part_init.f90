      ...
      subroutine load_foreshock_Maxwellian(vth,Ni_tot_1,Ni_tot_2,mass,population)
            ...
            do l = Ni_tot_1,Ni_tot_2
                  ...
                  if ((population .eq. 1) .or. (population .eq. 5) .or. (population .eq. 0) .or. (population .eq. 4)) then !Solar Wind, Left Edge
                        ...
                        vp(l,1) = -FSDriftSpeed*va_f*va*( b1(i,j,k,1) / &
                        (  sqrt( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) ) ) + &
                        FSThermalRatio*vx + ( EvBy*b1(i,j,k,3) - EvBz*b1(i,j,k,2) ) / &
                        ( ( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) )
                        vp(l,2) = -FSDriftSpeed*va_f*va*( b1(i,j,k,2) / &
                        (  sqrt( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) ) ) + &
                        FSThermalRatio*vy - ( EvBx*b1(i,j,k,3) - EvBz*b1(i,j,k,1) ) / &
                        ( ( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) )
                        vp(l,3) = -FSDriftSpeed*va_f*va*( b1(i,j,k,3) / &
                        (  sqrt( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) ) ) + &
                        FSThermalRatio*vz + ( EvBx*b1(i,j,k,2) - EvBy*b1(i,j,k,1) ) / &
                        ( ( b1(i,j,k,1)**2+b1(i,j,k,2)**2+b1(i,j,k,3)**2 ) )
